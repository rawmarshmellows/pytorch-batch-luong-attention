...
class EncoderRNN(nn.Module):
    def __init__(self, hidden_size, input_size, n_layers, dropout, word_embedding_matrix, rnn_cell, should_mask, use_cuda):
        super().__init__()
        ...
        if rnn_cell == "GRU":
            self.rnn = nn.GRU(input_size, hidden_size, n_layers, dropout, bidirectional=True)
    ...
    def forward(self, input_seqs, hidden, input_lengths):
        ...
        #     outputs, hidden = masked_rnn_cell(self.gru, outputs, input_lengths, None, True, self.use_cuda)
...
class LuongAttnDecoderRNN(nn.Module):
    def __init__(self,
        ...
        self.embedding = word_embedding_matrix
        if rnn_cell == "GRU":
            self.rnn = nn.GRU(input_size, hidden_size, n_layers, dropout)
        elif rnn_cell == "SRU":
            self.rnn = SRU(input_size, hidden_size, n_layers, dropout, use_tanh=1)
    ...
    def forward(self,
        ...
        decoder_outputs, decoder_hidden = self.rnn(packed, sorted_encoder_hidden)
